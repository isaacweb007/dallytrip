 ...
 * 사진과 최저가는 이미 돌고 있는 hotels-search에서 가져오고, 하루치를 브라우저에 캐시한다.
 *
...
(() => {
  ...
  async function fillPhoto(c) {
    ...
    const put = (v) => {
      if (!v || !v.thumb || pick(back).q !== c.q) return;      // 그새 다른 날짜로 넘겼으면 무시
      box.insertAdjacentHTML('afterbegin', `<img src="${esc(v.thumb)}" alt="${esc(c.city)}">`);
      if (v.from) box.insertAdjacentHTML('beforeend', `<div class="td-price">${esc(v.from)}부터</div>`);
    };
    ...
    try {
      ...
      const withPhoto = (r.hotels || []).find((h) => h.thumb);
      if (!withPhoto) return;
      const cheapest = (r.hotels || []).reduce((a, b) => (a.total_usd <= b.total_usd ? a : b));
      const v = { thumb: withPhoto.thumb, from: `$${Math.round(cheapest.total_usd / 2)}` };  // 1박 기준
