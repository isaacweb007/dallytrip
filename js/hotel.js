...
(() => {
  ...
  const CATEGORY = {
    ...
    Facilities: '시설', Value: '가격 대비', Food: '식사', Comfort: '편안함', Staff: '직원',
    'Value for money': '가격 대비', Breakfast: '조식', Amenities: '편의시설',
  };
  ...
  function verdict(h, s) {
    if (!h.rating) return '';
    const cats = (s.categories || []).filter((c) => c.name && c.rating);
    return `<section>
